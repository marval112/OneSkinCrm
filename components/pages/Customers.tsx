       ...
       <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
        <div className="flex items-center gap-4">
            {selectedCustomers.length > 0 && view === 'table' ? (
                <button onClick={() => setConfirmDelete(true)} className="px-4 py-2 bg-danger text-white rounded-md hover:bg-danger-hover">
        ...
        <div className="flex items-center gap-2">
            {user?.role === 'Admin' && (
                <button onClick={() => setIsImportModalOpen(true)} className="px-4 py-2 bg-success text-white rounded-md hover:bg-success-hover">{t('customers.importExcel')}</button>
            )}
            <button onClick={handleExport} className="px-4 py-2 bg-slate-600 text-white rounded-md hover:bg-slate-700">{t('customers.exportExcel')}</button>
            <button onClick={() => setIsCreateModalOpen(true)} className="w-full md:w-auto px-4 py-2 bg-primary text-white font-semibold rounded-md hover:bg-primary-hover whitespace-nowrap">
      ...
      {view === 'table' ? (
        <div className="overflow-x-auto">
            ...
            <thead className="bg-slate-50 dark:bg-slate-700/50">
                <tr>
                <th className="px-6 py-3"><input type="checkbox" onChange={handleSelectAll} checked={selectedCustomers.length > 0 && selectedCustomers.length === sortedCustomers.length} /></th>
            ...
            <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                ...
                <tr key={customer.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap hidden md:table-cell">
                        {customer.segment && <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800 dark:bg-gray-900/50 dark:text-gray-300">{customer.segment}</span>}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusColors[customer.status]}`}>{customer.status}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap hidden lg:table-cell">
                        <div className="flex items-center">
                            <div className="w-20 bg-gray-200 dark:bg-slate-600 rounded-full h-2.5"><div className="bg-success h-2.5 rounded-full" style={{ width: `${customer.health_score}%` }}></div></div>
