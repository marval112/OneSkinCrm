...
`;
  try {
    const response = await client.models.generateContent({ model: 'gemini-1.5-flash-001', contents: prompt });
    return response.text;
  } catch (e) {
...
`;
  try {
    const response = await client.models.generateContent({ model: 'gemini-1.5-flash-001', contents: prompt, config: { responseMimeType: 'application/json' } });
    const text = response.text.trim();
...
Tasks: ${JSON.stringify(tasks.slice(0, 40))}`;
  try { const r = await client.models.generateContent({ model: 'gemini-1.5-flash-001', contents: prompt, config: { responseMimeType: 'application/json' } }); const arr = JSON.parse(r.text.trim()); if (Array.isArray(arr)) return arr as AgendaItem[]; throw new Error('bad'); } catch { return []; }
};
...
Recent activity:\n${recent || '(none)'}
`;
  try { const r = await client.models.generateContent({ model: 'gemini-1.5-flash-001', contents: prompt }); return r.text; } catch { return 'Could not generate strategy.'; }
};
...
Recent activity:\n${recent || '(none)'}
`;
  try { const r = await client.models.generateContent({ model: 'gemini-1.5-flash-001', contents: prompt, config: { responseMimeType: 'application/json' } }); const arr = JSON.parse(r.text.trim()); if (Array.isArray(arr)) return arr as SuggestedTask[]; throw new Error('bad'); } catch { return []; }
};
...
`;
  try { const r = await client.models.generateContent({ model: 'gemini-1.5-flash-001', contents: prompt }); return r.text; } catch { return 'No se pudieron generar insights.'; }
};
...
`;
  try {
    const r = await client.models.generateContent({ model: 'gemini-1.5-flash-001', contents: prompt, config: { responseMimeType: 'application/json' } });
...
Recent activity:\n${recent || '(none)'}
`;
  try { const r = await client.models.generateContent({ model: 'gemini-1.5-flash-001', contents: prompt }); return r.text; } catch { return 'Could not generate summary.'; }
};
...
`;
  try { const r = await client.models.generateContent({ model: 'gemini-1.5-flash-001', contents: prompt, config: { responseMimeType: 'application/json' } }); const obj = JSON.parse(r.text.trim()); if (obj.subject && obj.body) return obj; throw new Error('bad'); } catch { return { subject: 'Seguimiento', body: 'Hola,\n\nQuería compartir novedades y coordinar próximos pasos.\n\nSaludos,' }; }
};
...
`;
  try { const r = await client.models.generateContent({ model: 'gemini-1.5-flash-001', contents: prompt, config: { responseMimeType: 'application/json' } }); const arr = JSON.parse(r.text.trim()); if (Array.isArray(arr)) return arr as number[]; throw new Error('bad'); } catch { return tasks.map(t => t.id); }
};
